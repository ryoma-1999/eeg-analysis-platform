...
論理的な処理の流れは、FrontendからBackendへ解析を依頼し、Backendが信号処理を実行して結果をFrontendへ返す構成である。

FrontendはFrontendコンテナから配信され、利用者のWebブラウザ内で実行される。そのため、実際のREST APIリクエストはブラウザ内で動作するReactからBackendへ送信される。
